ActiveAdmin.register_page 'Calendar Dashboard' do
  ...
  content do
    ...
    table id: 'calendar-table',
          style: 'width: 100%; table-layout: fixed; border-collapse: separate; border-spacing: 8px; border: 1px solid #ddd;' do
      thead do
        tr do
          %w[Sunday Monday Tuesday Wednesday Thursday Friday Saturday].each_with_index do |day, _index|
          ...
          tr do
            week.each_with_index do |date_day, index|
              orders_on_this_day = orders.select do |order|
                [order.finish, order.first_fitting, order.second_fitting, order.third_fitting,
                 order.fourth_fitting].include?(date_day)
              end
