    ...
    if current_user.role == "Administrator" || current_user.role == "Master Tailor" || current_user.role == "Sales Assistant" || current_user.role == "Production Manager" || current_user.role == "Coat Maker"
      ...
      @coats.each do |coat|
        ...
        cbody = [
          ...
          [{image: "#{dir}pockets_6.png",  scale: 0.25, colspan: 2}, {image: "#{dir}pockets_7.png",  scale: 0.25, colspan: 2}, "Vent: #{coat&.vent}", "Jacket length: #{coat&.jacket_length}", "Back Width: #{coat&.back_width}", "Sleeves: #{coat&.sleeves}", "Cuffs: #{coat&.cuffs_1}/#{coat&.cuffs_2}", "Collar: #{coat&.collar}"],
